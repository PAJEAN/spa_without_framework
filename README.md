...
<p align="justify">
The application makes a ajax call to get fake todos on <a href="https://jsonplaceholder.typicode.com/guide.html">jsonplaceholder</a>, json return includes 200 todos from 10 different users (20 todos / user).
</p>
...
<p align="justify">
<i>customElements</i> specifications offer to us a really good way to design reusable components. Indeed, <i>customElements</i> object allow us to build our own HTML tags with their own logic and allowing us to access to life cycle functions of tags to control what the elements does when it created and when it destroyed. I advise you a good reading by <a href="https://developers.google.com/web/fundamentals/web-components/customelements">Eric Bidelman</a>. Also, one of the particular benefit is to use shadow root and template tag (with slot) to <u>isolate</u> and stylize our element. I refer you to the Google Chrome Labs post <a href="https://github.com/GoogleChromeLabs/howto-components">howto-components</a> (an enriching read).
</p>
...
<p align="justify">
<i>Nota Bene</i>, we can use css variables of the main css file (here: "./styles/style.css") can also be used inside on it.
</p>

<p align="justify">
<i>customElements</i> are universel (it can be included in any recent JS project) and it can be seen like a React component or a Vue component. In my project, I try, as I can, to differenciate reusable <i>UI elements</i> ("./js/components/ui/"), <i>elements dedicated</i> to the application ("./js/components/") and <i>pages</i> ("./js/pages/").
...
Concerning elements dedicated to the application, usually it implements my store to dispatch some actions (to edit my state through mutations) and it subscribes to modifications of the state to re-render the view.
...
<p align="justify">
The state manager used for this project is inpired by the article of Andy Bell (<a href="https://css-tricks.com/build-a-state-management-system-with-vanilla-javascript/">ref</a>) on "css-tricks.com". It follows the logic of Vuex where:
<ul>
<li> <i>components</i> can dispatch actions (and subscribe to events) </li>
<li> <i>actions</i> call mutations and can do some asynchrone API calls </li>
<li> <i>mutations</i> change the state which is the single source of truth </li>
</ul>
To make a reactive components, the state is wrapped into a <i>Proxy</i> Javascript object that publish a new event when the state is update (with the PubSub class). So, all components subscribed to this event can catch it to re-render their view (<i>Nota bene</i>: To call the handle "set" function of a Proxy and be reactive, a reassignment of the state is necessary).
</p>
...
<p align="justify">
I made some adaptations to the initial code. First, "<i>subscribe()</i>" method of the PubSub class return the unsubscribe method. Then, returns of actions is now a Promise to know in the component context when the action is done. Finally, I added <u>getters</u> and <u>modules</u> notions to the state manager.
...
<p align="justify">
Thus a getter have to be a function with two parameters: state and key where state is a reference of our state and key is the name of our getter.
</p>
...
<p align="justify">
To add modules, I edited the intialisation of the store to add state, actions, mutations and getters of my module. I kept "state.js", "actions.js" and "mutations.js" files to allow me to put all things unrelated to a specific module.
</p>
...
<p align="justify">
Clarification, module notion is near to the Vuex module notion but in this implementation I don't have the concept of <i>local and root state</i>. However to be sure, that all keys of state, actions, mutations and getters objects is unique, I applied a namespacing with the dynamic keys name property of objects.
</p>
...
<p align="justify">
The router is inspired by the Jasim implementation (<a href="https://medium.com/@jasim/declarative-router-with-web-components-43ddcebc9dbc">ref</a>). I just adapted some pieces of code. Fisrt, I didn't use the "<i>pushState()</i>" function, I opted to a "#" URL style. The main reason is about the initial "<i>updateLinks()</i>" method. This method can't update links when pages make asynchone calls. Possible solutions are to deport asynchrone calls out of pages or to use a "MutationObserver" object to check when a specific attribute on the component page (<i>e.g.</i> "is-loading") change his value. Second adaptation was to add an authtication/role checking to some pages (but I need to make some additional tests for this functionality). Finally, I defined a "is-default" attribute to redirect users in a default page when a link doesn't exit.
</p>
